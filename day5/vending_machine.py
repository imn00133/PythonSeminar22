# 자판기의 종료, 돈 입력 같은 제어명령를 저장한 딕셔너리 상수를 넣음
CONTROL_STR = {
    'admin': ('물품출력', '개수추가', '종료'),
    'normal': ('돈 입력', '거스름돈', '종료')
}
...
def min_item_price(item_list):
    ...
    """
    min_price = item_list[0]['가격']
    for item in item_list:
        if min_price > item['가격']:
            min_price = item['가격']
    return min_price
...
def calc_ctrl_str_index(item_list, searching_value, permission='normal'):
    """
    ...
    :return 제어 명령의 위치
    """
    return len(item_list) + CONTROL_STR[permission].index(searching_value) + 1


def print_ctrl_str(item_list, permission='normal'):
    """
    ...
    if permission == 'admin':
        item_count = 0
    for index, control_content in enumerate(CONTROL_STR[permission], item_count + 1):
...
def print_item(item_list, permission='normal'):
    ...
    for index in range(len(item_list)):
        print("{0}. {1}({2}원)".format(index + 1, item_list[index].get('물품'), item_list[index]['가격']), end='')
        if permission == 'admin':
            print(" 개수: {0}".format(item_list[index]['개수']), end='')

        print("")
    if permission != 'admin':
        print_ctrl_str(item_list)
...
def admin_mode(item_list):
    ...
    while True:
        print("")
        print_ctrl_str(permission='admin')
        select_value = int(input("원하는 작업을 선택해주세요: "))
        print("")
        if select_value == calc_ctrl_str_index({}, '물품출력', 'admin'):
            print_item(item_list, permission='admin')
        elif select_value == calc_ctrl_str_index({}, '개수추가', 'admin'):
            print_item(item_list, permission='admin')
            item_select = int(input("개수를 추가할 물품을 선택하세요: "))
            if 0 < item_select <= len(item_list):
                item_list[item_select - 1]['개수'] += int(input("추가할 개수를 입력해주세요: "))
                continue
            print("물품을 잘못 선택하셨습니다.")
        elif select_value == calc_ctrl_str_index({}, '종료', 'admin'):
            print("자판기 모드로 돌아갑니다.")
            return item_list
        else:
            print("잘못 입력하셨습니다.")
...
while True:
    print("")
    select_value = ''
    price_min = min_item_price(item_list)
    ...
    # 돈 입력 및 거스름돈을 선택했을 때
    if select_value == calc_ctrl_str_index(item_list, '거스름돈'):
        total_money = return_charge(total_money)

    elif select_value == calc_ctrl_str_index(item_list, '돈 입력'):
    ...
    # 종료를 누르면 종료
    elif select_value == calc_ctrl_str_index(item_list, '종료'):
        break
